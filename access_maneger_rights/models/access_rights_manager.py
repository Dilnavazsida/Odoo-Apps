...
class AccessRightsManager(models.Model):
    ...
    @api.model
    def _arm_fill_hidden_menus(self, config, rules):
        """Collect hidden menus, their children, and the window actions they open.

        Hiding a menu only removes it from the navbar. Login/refresh still restores
        the last URL action (e.g. Sales Quotations), so JS needs those action ids.
        """
        selected_ids = list(set(rules.mapped("menu_line_ids.menu_id").ids))
        if not selected_ids:
            return
        Menu = self.env["ir.ui.menu"].sudo().with_context(
            arm_skip_rules=True, active_test=False
        )
        menus = Menu.search([("id", "child_of", selected_ids)])
        config["hidden_menu_ids"] = menus.ids
        ...
        for menu in menus:
            ...
            if xmlid:
                xmlids.add(xmlid)
            path = getattr(action, "path", False)
            if path:
                paths.add(path)
        config["hidden_action_ids"] = list(action_ids)
        config["hidden_action_xmlids"] = list(xmlids)
        config["hidden_action_paths"] = list(paths)

    @api.model
    def _empty_access_config(self):
        return {
            "is_readonly": False,
    ...
    def get_user_access_config(self):
        ...
        if self.env.context.get("arm_computing_config"):
            return self._empty_access_config()
        self = self.with_context(arm_computing_config=True)
        rules = self._get_rules_for_user()
        config = self._empty_access_config()
        if not rules:
            return config
        ...
        self._arm_fill_hidden_menus(config, rules)
